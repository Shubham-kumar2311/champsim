...
uint64_t nvm_writebacks = 0;
uint64_t dram_writebacks = 0;
...
bool CACHE::handle_fill(const mshr_type& fill_mshr)
{
  ...
  if (way != set_end) {
    ...
    *way = fill_block(fill_mshr, metadata_thru);
    uint32_t channel = (fill_mshr.address.to<uint64_t>() >> 6) % 4;

    if (channel == 0)
        way->memory_type = 0; // DRAM
    else
...
void CACHE::end_phase(unsigned finished_cpu)
{
  ...
  for (auto* ul : upper_levels) {
    ...
    if (!warmup && !printed) {
        std::cout << "------------------------------" << std::endl;
        std::cout << "------------------------------" << std::endl;
        std::cout << "NVM Writebacks: " << nvm_writebacks << std::endl;
        std::cout << "DRAM Writebacks: " << dram_writebacks << std::endl;
        std::cout << "------------------------------" << std::endl;
        std::cout << "------------------------------" << std::endl;
        printed = true;
    }
